...
//HostOpenConsole();
	...
	if (PlayitemCheck(path))
	{
		...
		string linkApi = "https://www.youtube.com/get_video_info?video_id=" + videoId + "&eurl=https://youtube.googleapis.com/v/" + videoId + "&html5=1&c=TVHTML5&cver=6.20180913"; // info, embedded, detailpage, vevo
		...
		for (int z = 0; z < 2; z++)
		{
			...
			else
			{
				...
				{
					LoadAPI = true;
					ApiData = HostUrlGetString(linkApi, GetUserAgent());
				}
				strData = ApiData;
			}
			...
			else
			{
				...
				player_response_jsonData.replace("\\\"", "\"");
				player_response_jsonData.replace("\\\\", "\\");
				if (player_response_jsonData.empty()) player_response_jsonData = GetJsonCode(strData, MATCH_PLAYER_RESPONSE_2);								

				player_chapter_jsonData = GetJsonCode(strData, MATCH_CHAPTER_RESPONSE);
				if (player_chapter_jsonData.empty()) player_chapter_jsonData = GetJsonCode(strData, MATCH_CHAPTER_RESPONSE2);
			...
			if (player_response_jsonData.empty() && stream_map_len <= 0 && hlsmpd_len <= 0) continue;
			...
			else
			{
				...
				string DashMPD;
				if (adaptive_fmts_start <= 0 && dashmpd_len > 0) DashMPD = strData.substr(dashmpd_start, dashmpd_len); // dash 포멧 이라면.. ㄷㄷㄷ			
				if (!DashMPD.empty())
				{
